...
export default function Dashboard() {
  ...
  return (
    <div className="page-shell max-w-none space-y-8 pb-12 text-[#30313A]">
      <style>{`
        .dashboard-trend-card .recharts-area-curve { stroke: #1976D2 !important; }
        .dashboard-trend-card .recharts-area-area { fill: #1976D2 !important; fill-opacity: .10 !important; }
        .dashboard-trend-card .recharts-cartesian-grid-horizontal line,
        .dashboard-trend-card .recharts-cartesian-grid-vertical line { stroke: #D9E1E7 !important; }
        .dashboard-trend-card .recharts-cartesian-axis-tick-value { fill: #667685 !important; }
        .dashboard-trend-card .bg-slate-100 { background: #F1F4F6 !important; }
        .dashboard-trend-card .text-slate-500,
        .dashboard-trend-card .text-slate-400 { color: #667685 !important; }
        .dashboard-trend-card .text-slate-900 { color: #17212B !important; }
        .dashboard-trend-card .text-blue-600 { color: #1976D2 !important; }
        .dashboard-trend-card button.bg-white { background: #EAF5FC !important; color: #1976D2 !important; }
        .dashboard-trend-card [class*="bg-[#403946]"] { background: #FFFFFF !important; border: 1px solid #D9E1E7 !important; color: #17212B !important; }
        .dashboard-trend-card [class*="text-slate-400"] { color: #667685 !important; }
        .dashboard-trend-card [class*="text-slate-200"] { color: #17212B !important; }
        .dashboard-trend-card [class*="text-blue-400"] { color: #1976D2 !important; }
        .dashboard-trend-card [class*="bg-[#F2EEE8]"] { background: #F1F4F6 !important; border-color: #D9E1E7 !important; color: #1976D2 !important; }
        .dashboard-trend-card [class*="text-[#3F3D45]"] { color: #17212B !important; }
        .dashboard-trend-card [class*="text-[#858087]"] { color: #667685 !important; }
      `}</style>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        {/* National Index Hero */}
        <div className="relative overflow-hidden rounded-3xl border border-[#D5E0E8] bg-[#EAF1F6] p-7 text-[#17212B] shadow-[0_12px_28px_rgba(23,33,43,.06)] lg:col-span-7 flex flex-col justify-between">
          <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-[#B9D9EE]/25 blur-3xl" />
          <div className="absolute -bottom-24 -left-16 h-52 w-52 rounded-full bg-[#D7E0E8]/40 blur-3xl" />
          ...
          <div className="relative">
            ...
            <div className="mt-6 flex items-baseline gap-4">
              <span className="text-[72px] font-bold leading-none tracking-tighter text-[#17212B]">
                {latestNational ? latestNational.index.toFixed(2) : '100.00'}
              </span>
              <span className="text-sm font-semibold text-[#667685]">pts</span>
            </div>
          ...
          <div className="relative mt-8 grid grid-cols-1 gap-4 border-t border-[#D5E0E8] pt-5 sm:grid-cols-2">
            <div>
              <div className="mb-1 flex justify-between text-xs font-semibold text-[#17212B]">
                <span>Weighted Route Coverage</span>
                <span className="font-bold text-[#17212B]">{formatNumber(nationalCoverage * 100, 1)}%</span>
              </div>
              <div className="mb-2 h-2 w-full overflow-hidden rounded-full bg-[#D7E0E8]">
                <div className="h-full rounded-full bg-[#1976D2] transition-all duration-1000" style={{ width: `${Math.min(100, nationalCoverage * 100)}%` }} />
              </div>
              <p className="text-[11px] leading-normal text-[#667685]">
            ...
            <div>
              <div className="mb-1 text-xs font-semibold text-[#17212B]">Observations Used</div>
              <div className="mb-1 text-xl font-bold text-[#17212B]">{formatNumber(latestNational?.observations_used || 0)}</div>
              <p className="text-[11px] leading-normal text-[#667685]">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white border border-[#D9E1E7] p-7 shadow-[0_12px_28px_rgba(23,33,43,.06)] lg:col-span-5 flex flex-col justify-between">
          <div>
            <div className="flex items-start justify-between">
              <div>
                <div className="text-[11px] font-bold uppercase tracking-widest text-[#667685]">
                  Reference Route Signal
                </div>
                <div className="mt-2 flex items-center gap-2">
                  <span className="rounded-lg bg-[#EEF3F7] px-3 py-1 text-sm font-bold text-[#17212B] border border-[#D7E0E8]">DEL</span>
                  <span className="text-[#667685] font-bold">→</span>
                  <span className="rounded-lg bg-[#EEF3F7] px-3 py-1 text-sm font-bold text-[#17212B] border border-[#D7E0E8]">BOM</span>
                </div>
              </div>
              
              {routeChange !== null && (
                <div className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold ${routeChange >= 0 ? 'bg-[#FEF2F2] text-[#DC2626] border border-[#F2B8B5]' : 'bg-[#EAF5EA] text-[#2E7D32] border border-[#B7D7B9]'}`}>
            ...
            <div className="mt-6 flex items-baseline gap-3">
              <span className="text-[60px] font-bold leading-none tracking-tighter text-[#17212B]">
                {latestRoute ? latestRoute.index.toFixed(2) : '100.00'}
              </span>
              <span className="text-xs font-semibold text-[#667685]">{formatNumber(latestRoute?.observations_used || 0)} valid observations</span>
            </div>

            <div className="mt-6 rounded-2xl bg-[#F1F4F6] border border-[#D9E1E7] p-4 text-xs text-[#667685] leading-relaxed">
              <div className="font-bold text-[#17212B] mb-1">Route Signal Mechanism</div>
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-xs font-medium text-[#667685] border-t border-[#D9E1E7] pt-4">
            <span className="rounded-md bg-[#EEF3F7] px-2.5 py-1 text-[#172B3A] border border-[#D7E0E8] font-semibold">Non-stop Directs</span>
            <span className="rounded-md bg-[#EEF3F7] px-2.5 py-1 text-[#172B3A] border border-[#D7E0E8] font-semibold">Advance: T+1 to T+45</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-3xl bg-white border border-[#D9E1E7] p-7 shadow-sm lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-[10px] font-bold tracking-widest uppercase text-[#667685] mb-1">
              <Calendar className="h-4 w-4 text-[#1976D2]" /> Pricing Behavior
            </div>
            ...
            <div className="space-y-4">
              {advancePurchaseSummary.map((item) => (
                <div key={item.advanceDays} className="flex items-center gap-4">
                  <div className="w-12 text-xs font-bold text-[#17212B]">T+{item.advanceDays}</div>
                  <div className="flex-1 h-3 rounded-full bg-[#E1E8EE] overflow-hidden relative">
                    <div 
                      className="absolute top-0 left-0 h-full rounded-full bg-[#1976D2] transition-all duration-1000" 
                      style={{ width: item.average && maxAdvanceAvg ? `${(item.average / maxAdvanceAvg) * 100}%` : '0%' }} 
                    />
                  </div>
                  <div className="w-28 text-right">
                    {item.average !== null ? (
                      <div className="text-sm font-bold text-[#17212B]">{formatCurrency(item.average)}</div>
                    ) : (
                      <div className="text-xs font-medium text-[#667685]">Awaiting data</div>
                    )}
                    <div className="text-[10px] font-medium text-[#667685]">{item.observations} valid obs</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={label} className="rounded-2xl bg-[#F1F4F6] border border-[#D9E1E7] p-4 flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider text-[#667685]">{label}</div>
                    <div className="text-[10px] font-medium text-[#667685] mt-0.5">{row ? formatDate(row.period_start) : 'Base sequence'}</div>
                  </div>
                  <div className="text-right">
                    {row ? (
                      <div className="text-xl font-bold text-[#17212B]">{row.index.toFixed(2)}</div>
                    ) : (
                      <div className="text-xs font-semibold text-[#667685]">100.00 (Base)</div>
                    )}
                  </div>
                </div>
              ))}
